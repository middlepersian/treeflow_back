        ...
        if input.get('id', None) is None:
            ...
            if input.get('text', None) is not None:
                if Text.objects.filter(pk=from_global_id(input['text']['id'])[1]).exists():
                    ...
                    return cls(token=None, success=False, errors=["Text with ID {} not found".format(input['text']['id'])])
            ...
            if input.get('line', None) is not None:
                ...
                if Line.objects.filter(pk=from_global_id(input['line']['id'])[1]).exists():
                    line = Line.objects.filter(pk=from_global_id(input['line']['id'])[1]).first()
                    token.line = line
                else:
