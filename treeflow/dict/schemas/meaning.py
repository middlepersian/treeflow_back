...
from treeflow.dict.types.meaning import (
    Meaning,
    MeaningInput,
    MeaningPartial,
    MeaningElastic,
...
@gql.type
class Mutation:
    create_meaning: Meaning = gql.django.create_mutation(
        MeaningInput, directives=[IsAuthenticated()]
    )
    update_meaning: Meaning = gql.django.update_mutation(
        MeaningPartial, directives=[IsAuthenticated()]
    )
    delete_meaning: Meaning = gql.django.delete_mutation(
        gql.NodeInput, directives=[IsAuthenticated()]
    )

    @gql.django.input_mutation
    def add_related_meaning_to_meaning(
        self, info, meaning: relay.GlobalID, related: relay.GlobalID
    ) -> Meaning:
