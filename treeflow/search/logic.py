...
def retrieve_initial_anchors(criterion: TokenSearchInput) -> List[Token]:
    ...
    if criterion.query_type == "exact":
        ...
        tokens = Token.objects.filter(
            **{f"{criterion.field}__icontains": criterion.value}
        )
...
def filter_sections_by_logic(
    candidate_sections, token_search_inputs: List[TokenSearchInput]
):
    ...
    for criterion in token_search_inputs[1:]:
        ...
        if criterion.distance:
            for anchor in initial_anchors:
                ...
                candidate_sections = apply_distance_constraints(
                    anchor, candidate_sections, criterion.distance
                )
